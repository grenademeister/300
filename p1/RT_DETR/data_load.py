        ...
        if self.transform:
            if type(idx) == type(1):
                transformed = self.transform(
                    image=image, bboxes=boxes, category=categories
                )
                ...
                for obj_index in range(len(image)):
                    transformed = self.transform(
                        image=image[obj_index],
                        bboxes=boxes[obj_index],
                        category=categories[obj_index],
                    )
            ...
            if type(idx) == type(1):
                formatted_annotations.append(
                    self.format_image_annotations_as_coco(image_id, categories, boxes)
                )
            else:
                for obj_index in range(len(image)):
                    formatted_annotations.append(
